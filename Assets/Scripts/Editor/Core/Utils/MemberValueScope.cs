...
namespace UnityEditor
{
    internal readonly ref struct MemberValueScope<T>
    {
        ...
        public MemberValueScope(Expression<Func<T>> fieldOrProp, T tempValue) : this(fieldOrProp, tempValue, false)
        {
        }

        public MemberValueScope(Expression<Func<T>> fieldOrProp, Func<T, T> tempValueFactory) : this(fieldOrProp, tempValueFactory, true)
        {
        }
        ...
        private static void UpdateMemberValue(MemberInfo member, object targetObj, object newValue, out object prevValue, bool newValueFromFactory)
        {
            switch (member)
            {
